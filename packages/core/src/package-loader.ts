...
async function readManifest(packageDir: string): Promise<Result<PackageManifest, LoadError>> {
  ...
  try {
    ...
    if (cause.code === 'ENOENT') {
      return {
        ok: false,
        error: { type: 'manifest_missing', message: `manifest.yaml not found in ${packageDir}` }
      };
    }
    return {
      ok: false,
      error: { type: 'read_error', message: `Failed to read manifest.yaml: ${cause.message}` }
    };
  }
  ...
  try {
    ...
    return {
      ...
      error: {
        ...
        message: `Invalid YAML in manifest.yaml: ${cause.message}`
      }
    };
  }
    ...
    for (const chapter of manifestResult.value.chapters) {
      if (!isValidChapterPath(normalizedPackageDir, chapter.file)) {
        return {
          ...
          error: {
            ...
            message: `Chapter '${chapter.id}' uses an invalid or unsafe path: ${chapter.file}`
          }
        };
      }
      ...
      if (!chapterPath) {
        return {
          ...
          error: {
            type: 'chapter_invalid_path',
            message: `Chapter '${chapter.id}' resolves outside the package or does not exist: ${chapter.file}`
          }
        };
      }
      ...
      try {
        ...
        if (!stat.isFile()) {
          return {
            ...
            error: {
              ...
              message: `Chapter file is not a file: ${chapter.file}`
            }
          };
        }
      } catch {
        return {
          ok: false,
          error: { type: 'chapter_missing', message: `Chapter file not found: ${chapter.file}` }
        };
      }
    }
...
export async function readChapter(
  ...
  if (!isValidChapterPath(normalizedPackageDir, chapterFile)) {
    return {
      ok: false,
      error: { type: 'chapter_invalid_path', message: `Unsafe chapter path: ${chapterFile}` }
    };
  }

  const chapterPath = await resolveSafeExistingPath(
    path.join(normalizedPackageDir, 'chapters'),
    chapterFile,
    normalizedPackageDir
  );
  if (!chapterPath) {
    return {
      ...
      error: {
        type: 'chapter_invalid_path',
        message: `Chapter path is missing or escapes the package: ${chapterFile}`
      }
    };
  }
  ...
  try {
    ...
    if (!stat.isFile())
      return {
        ok: false,
        error: { type: 'chapter_missing', message: `Chapter file is not a file: ${chapterFile}` }
      };
    ...
    return {
      ok: false,
      error: {
        type: cause.code === 'ENOENT' ? 'chapter_missing' : 'read_error',
        message: `Failed to read chapter: ${cause.message}`
      }
    };
  }
}
