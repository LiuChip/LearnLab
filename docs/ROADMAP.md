...
> 相关文档：[插件加载与 API 设计](superpowers/specs/2026-09-01-plugin-loading-and-scoped-permissions-design.md) · [总览](OVERVIEW.md) · [架构设计](ARCHITECTURE.md) · [安全机制](SECURITY.md) · [数据模型](DATA_MODEL.md) · [测试策略](TEST_STRATEGY.md)
...
> 当前基线：插件按“全局插件 + 当前实验包直接依赖及递归依赖链”解析；学习区用 `dependencies/` 统一复用实际依赖；核心只提供配置 JSON 导入/导出，不实现云同步。
...
- 解析包 `runtime_dependencies`，计算共享依赖指纹；
- 在当前学习区根目录创建/复用 `dependencies/` 的目标路径，但暂不自动安装系统级依赖。
...
| W6 | 学习区和依赖 | `workspace.db`、包列表、切换、最近打开、`dependencies/` 和指纹记录 |
...
3. **纯依赖路径管理**：实现依赖指纹和 `<workspace>/dependencies/<kind>/<fingerprint>` 路径计算；安装动作之后再接。
