...
2026-09-09 试用版补充：Menubar 和窗口控件只保留占位；所有 INFO 气泡统一独立计时并显示消失进度，气泡消失后可从会话通知中心查看；通知中心与终端/输出相互独立。侧栏尺寸、覆盖层断点、快速打开和搜索结果样式为可调整实现，不自动冻结。实现边界和审查详见 [审查记录](../../reviews/2026-09-09-ui-prototype-review.md)。
